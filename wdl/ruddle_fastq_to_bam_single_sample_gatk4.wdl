...
task FastqToUbam {
  ...
  command {
    java -Xmx4G -jar $EBROOTPICARD/picard.jar FastqToSam \
    FASTQ=${fastq1} \
    FASTQ2=${fastq2} \
...
task SamToFastqAndBwaMem {
  ...
  command <<<
    ...
    bash_ref_fasta=${ref_fasta}
		...
		java -Dsamjdk.compression_level=${compression_level} -Xms3000m -jar $EBROOTPICARD/picard.jar \
      SamToFastq \
			INPUT=${input_bam} \
			FASTQ=/dev/stdout \
			INTERLEAVE=true \
...
task MergeBamAlignment {
  ...
  command {
    # set the bash variable needed for the command-line
    bash_ref_fasta=${ref_fasta}
    gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xms15000m" \
      MergeBamAlignment \
...
task SortAndFixTags {
  ...
  command {
    ...
    gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xms30000m" \
      SortSam \
      --INPUT ${input_bam} \
      --OUTPUT /dev/stdout \
    ...
    | \
    gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xms500m" \
      SetNmAndUqTags \
      --INPUT /dev/stdin \
      --OUTPUT ${output_bam_basename}.bam \
      --CREATE_INDEX true \
...
task MarkDuplicates {
 ...
 # While query-grouped isn't actually query-sorted, it's good enough for MarkDuplicates with ASSUME_SORT_ORDER="queryname"
  command {
    gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xms32000m" \
      MarkDuplicates \
...
task GatherBamFiles {
  ...
  command {
    gatk --java-options "-Dsamjdk.compression_level=${compression_level} -Xms14000m" \
      GatherBamFiles \
      --INPUT ${sep=' --INPUT ' input_bams} \
      --OUTPUT ${output_bam_basename}.bam \
      --CREATE_INDEX true \
